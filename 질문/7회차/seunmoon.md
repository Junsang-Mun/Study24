...
- 트리(tree) - 이진 트리(binary tree)  
    ...
    - 이진 트리의 순회 절차 (주의! 순회는 모두 아래의 절차들을 사용하긴 하나 순회의 종류에 따라  순서가 다름)  
        - 1) 노드 방문(Visit the node) : 노드를 처리  
        - 2) 왼쪽 이동(Go Left) : 왼쪽 자식 노드로 이동  
        - 3) 오른쪽 이동(Go Right) : 오른쪽 자식 노드로 이동  
        
        
    - 이진 트리의 순회(traverse) 종류 (절차의 순서는 바로 윗 내용의 번호 참고)  
        - 전위 (pre-order) : 절차의 순서는 1 - 2 - 3
        - 중위 (in-order)  : 절차의 순서는 2 - 1 - 3
        - 후위 (post-roder)  : 절차의 순서는 2 - 3 - 1
